...
st.set_page_config(page_title="ConvoSpace", page_icon="💬", layout="centered")
st.title("ConvoSpace")

# ---- Models ----
if "MODELS" not in st.session_state:
    st.session_state.MODELS = get_ollama_models_list()
...
with st.sidebar:
    ...
    for cid, title in conversations.items():
        ...
        cols = st.columns([4, 1])
        if cols[0].button(label, key=f"conv_{cid}"):
        ...
        if cols[1].button("🗑️", key=f"del_{cid}"):
            ...
            if deleted:
                ...
                # Try to rerun; if not available, update query params as a fallback
                try:
                    ...
                    try:
                        ...
                        st.write("Conversation deleted — please refresh the page to see updates.")
